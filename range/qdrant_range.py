...
def search_queries(query_vectors, qdrantClient, collection_name, threshold):
...
def main():
    ...
    collection_name = "range_test"
    create_collection(qdrantClient, collection_name)

    insert_values(baseV, qdrantClient, collection_name)

    truth = utils.range_truth(pd.DataFrame({'vector': queryV.tolist()}), pd.DataFrame({'vector': baseV.tolist()}), threshold=600)

    start_time = time.time()
    threshold = 600
    result_ids = search_queries(queryV, qdrantClient, collection_name, threshold)
    end_time = time.time()
