...
def read_dataset():
    ...
    with open(os.getenv('PICKLE_GROUND_TRUTH_PATH_RANGE'), 'rb') as f:
        truth = pickle.load(f)
...
def main():
    ...
    print([len(elem) for elem in groundTruthV])
    # return
        ...
        for m in m_values:
            for ef in ef_values:
                ...
                for i,elem in enumerate(result_ids):
                    true_positives_iter = len(np.intersect1d(groundTruthV[i], result_ids[i]))
