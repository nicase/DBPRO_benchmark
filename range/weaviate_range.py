...
truth = utils.range_truth(query_vectors, base, threshold=300, function='euclidean')
# truth contains the IDs of the k nearest neighbors that also satisfy the attribute filtering clause


def run_query(name, max_distance, query_vec, base_vec):
    ...
    for i in range(len(result)):
        result_ids.append([])
        for j in range(len(result[i]["data"]["Get"][name])):
    ...
    with open("results_range_weviate.csv", mode='a') as file:
        writer = csv.writer(file)
        writer.writerow([name, k, (true_positives/n_classified), throughput, average_latency])

# Specify the file name
csv_file_name = "results_range_weviate.csv"
...
for i in names:
        run_query(i, 90000.0, query_vectors, base_vectors)

run_query("Siftsmall_flat_index", 90000.0, query_vectors, base_vectors)
