...
def create_HSNW_collections(ef, maxConnections, efConstruction, to_name):
    ...
    upload_data(class_name)
    for k_ in k_number:
        run_query(to_name, k_, query_vectors, base_vectors)
    client.schema.delete_class(class_name)
...
for i in ef:
    for j in maxConnections:
        for ef_cons in efConstruction:
            to_name = 'Siftsmall_HSNW_index' + '_' + str(i) + '_' + str(j) + '_' + str(ef_cons)
            create_HSNW_collections(i,j,ef_cons, to_name)
